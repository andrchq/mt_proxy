...
clear 
echo -e "${BLUE}===================================================${NC}"
echo -e "${BLUE}       MTProxy Установка          ${NC}"
echo -e "${BLUE}===================================================${NC}"

echo -e "\n${CYAN}Добро пожаловать в мастер установки MTProxy!${NC}"
echo -e "Этот скрипт поможет вам настроить и запустить свой Telegram прокси."
echo -e "Весь процесс займет не более 2-х минут.\n"

echo -e "${YELLOW}Этапы установки:${NC}"
echo -e "  1. Настройка порта"
echo -e "  2. Продвижение канала (спонсорский канал)"
echo -e "  3. Настройка домена"
echo -e "  4. Маскировка (Fake TLS)"
echo -e "  5. Установка и запуск\n"

read -p "Нажмите Enter, чтобы начать..." < /dev/tty
echo ""
...
if [[ "$1" == "uninstall" ]]; then
    ...
    read -p "Вы уверены, что хотите продолжить? (введите 'DA' для подтверждения): " CONFIRM < /dev/tty
...
echo -e "${CYAN}--- [1/5] Настройка Порта ---${NC}"
read -p "Использовать стандартный порт 443? (y/n): " USE_DEFAULT_PORT < /dev/tty
if [[ "$USE_DEFAULT_PORT" =~ ^[Yy]$ || -z "$USE_DEFAULT_PORT" ]]; then
    PORT=$DEFAULT_PORT
else
    read -p "Введите желаемый порт: " USER_PORT < /dev/tty
    PORT=${USER_PORT:-$DEFAULT_PORT}
fi
[[ -z "$PORT" ]] && PORT=$DEFAULT_PORT

echo -e "\n${CYAN}--- [2/5] Настройка Продвижения Канала ---${NC}"
echo -e "${YELLOW}💡 Подсказка:${NC} Вы можете рекламировать свой Telegram канал пользователям прокси."
read -p "Хотите настроить продвижение канала? (y/n): " SETUP_CHANNEL < /dev/tty
if [[ "$SETUP_CHANNEL" =~ ^[Yy]$ ]]; then
    read -p "Введите юзернейм канала (без @): " USER_CHANNEL < /dev/tty
...
# Настройка домена
echo -e "\n${CYAN}--- [4/5] Настройка Домена (Опционально) ---${NC}"
echo -e "${YELLOW}💡 Подсказка:${NC} Использование домена вместо IP повышает доверие и удобство."
echo -e "Если у вас есть домен, направленный на этот сервер ($EXTERNAL_IP), вы можете его использовать."
read -p "Хотите использовать домен? (y/n): " SETUP_DOMAIN < /dev/tty

if [[ "$SETUP_DOMAIN" =~ ^[Yy]$ ]]; then
    read -p "Введите доменное имя: " USER_DOMAIN < /dev/tty
else
    USER_DOMAIN=""
fi
...
# Настройка Fake TLS
echo -e "\n${CYAN}--- [5/5] Настройка Маскировки (Fake TLS) ---${NC}"
...
read -p "Использовать случайный домен для маскировки ($RANDOM_DOMAIN)? (y/n): " USE_RANDOM_TLS < /dev/tty
if [[ "$USE_RANDOM_TLS" =~ ^[Yy]$ || -z "$USE_RANDOM_TLS" ]]; then
    TLS_DOMAIN=$RANDOM_DOMAIN
else
    read -p "Введите домен для маскировки (например google.com): " USER_TLS_DOMAIN < /dev/tty
